...
async function initSecureDB() {
  ...
  try {
    ...
    await sql`CREATE TABLE IF NOT EXISTS admins (
      id SERIAL PRIMARY KEY, username VARCHAR(255) UNIQUE, pin VARCHAR(255), role VARCHAR(50)
    )`;

    // 2. جدول المنتجات
    await sql`CREATE TABLE IF NOT EXISTS products (
      id SERIAL PRIMARY KEY, name VARCHAR(255), price NUMERIC, old_price NUMERIC, 
      stock INT, sold INT DEFAULT 0, details TEXT, image TEXT, category VARCHAR(255), 
      is_sale BOOLEAN, out_of_stock BOOLEAN, modified_by VARCHAR(255)
    )`;

    // 3. جدول الأقسام
    await sql`CREATE TABLE IF NOT EXISTS categories (
      id SERIAL PRIMARY KEY, name VARCHAR(255), icon VARCHAR(50), parent VARCHAR(255)
    )`;

    // 4. جدول العمال
    await sql`CREATE TABLE IF NOT EXISTS workers (
      id SERIAL PRIMARY KEY, name VARCHAR(255), phone VARCHAR(255), details TEXT, 
      image TEXT, region VARCHAR(255), city VARCHAR(255), profession VARCHAR(255), 
      portfolio_img TEXT, safety_details TEXT, rating VARCHAR(50), is_busy BOOLEAN, 
      hidden BOOLEAN DEFAULT false, modified_by VARCHAR(255)
    )`;

    // 5. جدول الإعدادات
    await sql`CREATE TABLE IF NOT EXISTS settings (
      id SERIAL PRIMARY KEY, phone VARCHAR(255), email VARCHAR(255), shop_name VARCHAR(255)
    )`;

    // 6. جدول الطلبات
    await sql`CREATE TABLE IF NOT EXISTS orders (
      id SERIAL PRIMARY KEY, customer_name VARCHAR(255), customer_phone VARCHAR(255), 
      cart_data JSONB NOT NULL, total NUMERIC NOT NULL, status VARCHAR(50) DEFAULT 'معلق', 
...
// تشغيل السيرفر أولاً لتجنب مشاكل Render Port Scan
...
// Middleware للتحقق من الاتصال قبل معالجة الطلبات
...
app.post('/api/orders', async (req, res) => {
  ...
  try {
    const r = await sql`INSERT INTO orders (customer_name, customer_phone, cart_data, total) VALUES (${customer_name}, ${customer_phone}, ${cart_data}, ${total}) RETURNING *`;
...
// نقطة البيع: خصم المخزون عند البيع
...
// نقطة البيع: إرجاع بضاعة للمخزون
app.post('/api/pos/refund', async (req, res) => {
  try {
    const { cart, order_id } = req.body;
    for (let item of cart) {
      await sql`UPDATE products SET stock = stock + ${item.qty}, sold = GREATEST(0, sold - ${item.qty}) WHERE id = ${item.id}`;
    }
    if(order_id) await sql`DELETE FROM orders WHERE id = ${order_id}`;
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/products', async (req, res) => {
  const p = req.body;
  try {
    await sql`INSERT INTO products (name, price, old_price, stock, details, image, category, is_sale, out_of_stock, modified_by) 
    VALUES (${p.name}, ${p.price}, ${p.old_price}, ${p.stock}, ${p.details}, ${p.image}, ${p.category}, ${p.is_sale}, ${p.out_of_stock}, ${p.modified_by})`;
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.put('/api/products/:id', async (req, res) => {
  const p = req.body;
  try {
    await sql`UPDATE products SET name=${p.name}, price=${p.price}, old_price=${p.old_price}, stock=${p.stock}, 
    details=${p.details}, image=${p.image}, is_sale=${p.is_sale}, out_of_stock=${p.out_of_stock}, sold=${p.sold || 0}, modified_by=${p.modified_by}
...
app.post('/api/workers', async (req, res) => {
  const w = req.body;
  try {
    await sql`INSERT INTO workers (name, phone, details, image, region, city, profession, portfolio_img, safety_details, rating, is_busy, hidden, modified_by) 
    VALUES (${w.name}, ${w.phone}, ${w.details}, ${w.image}, ${w.region}, ${w.city}, ${w.profession}, ${w.portfolio_img}, ${w.safety_details}, ${w.rating}, ${w.is_busy}, false, ${w.modified_by})`;
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.put('/api/workers/:id', async (req, res) => {
  const w = req.body;
  try {
    await sql`UPDATE workers SET name=${w.name}, phone=${w.phone}, details=${w.details}, image=${w.image}, 
    region=${w.region}, city=${w.city}, profession=${w.profession}, portfolio_img=${w.portfolio_img}, 
    safety_details=${w.safety_details}, rating=${w.rating}, is_busy=${w.is_busy}, hidden=${w.hidden}, modified_by=${w.modified_by} 
    WHERE id = ${req.params.id}`;
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.put('/api/workers/:id/click', async (req, res) => {
  // يمكن استخدام هذا المسار لاحقاً لتسجيل إحصائيات النقر على العامل
  res.json({ success: true });
});
...
app.put('/api/settings', async (req, res) => {
  try {
    ...
    if (check.length > 0) {
      await sql`UPDATE settings SET shop_name=${req.body.shop_name}, phone=${req.body.phone} WHERE id = ${check[0].id}`;
    } else {
      await sql`INSERT INTO settings (shop_name, phone) VALUES (${req.body.shop_name}, ${req.body.phone})`;
    }
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});