...
function App() {
  ...
  const handleChangeMyPassword = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/admins/${currentUser.id}/pin`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ newPin: newPasswordInput }) });
      if (res.ok) { Swal.fire({title:'نجاح', text:'تم تغيير رمزك السري', icon:'success', position:'center'}); setNewPasswordInput(''); fetchAllData(); }
    } catch (error) { Swal.fire({title:'خطأ', text:'حدث خطأ', icon:'error', position:'center'}); }
  };
  ...
  const handleAddCategory = async (isSub = false) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/categories`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, icon: isSub ? '📂' : '📁', parent: isSub ? activeMainCat.name : '' }) });
      if (res.ok) { isSub ? setNewSubName('') : setNewMainName(''); Swal.fire({title:'نجاح', text:'تم الإضافة', icon:'success', position:'center', timer:1500}); fetchAllData(); }
    } catch(e) {}
  };
  ...
  const handleSaveProduct = async () => {
    ...
    try {
      ...
      if (res.ok) { Swal.fire({title:'نجاح', text:'تم حفظ المنتج بالمستودع', icon:'success', position:'center'}); setEditingItem(null); setFormData({ name: '', price: '', old_price: '', stock: '', details: '', manufacturer: '', color: '', warranty: '', badge: '', image: '', is_sale: false, out_of_stock: false }); fetchAllData(); }
    } catch (e) { Swal.fire({title:'خطأ', text:'فشل الاتصال', icon:'error', position:'center'}); }
  };
  ...
  const handleSaveWorker = async () => {
    ...
    try {
      ...
      if (res.ok) { Swal.fire({title:'نجاح', text:'تم الحفظ', icon:'success', position:'center'}); setWorkerForm({ name: '', phone: '', details: '', image: '', region: '', city: '', profession: '' }); setEditingWorker(null); fetchAllData(); }
    } catch(e) { Swal.fire({title:'خطأ', text:'فشل الاتصال', icon:'error', position:'center'}); }
  };
  ...
  const handleSaveAdmin = async () => {
    ...
    try {
      ...
      if (res.ok) { Swal.fire({title:'نجاح', text:'تم حفظ الموظف', icon:'success', position:'center'}); setNewAdminForm({ username: '', pin: '', role: 'موظف' }); setEditingAdmin(null); fetchAllData(); }
      else { Swal.fire({title:'خطأ', text:'الاسم مسجل مسبقاً', icon:'error', position:'center'}); }
    } catch (e) { Swal.fire({title:'خطأ', text:'فشل الاتصال', icon:'error', position:'center'}); }
  };
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      if (res.ok) { Swal.fire({title:'نجاح', text:'تم تحديث الإعدادات ✅', icon:'success', position:'center'}); fetchAllData(); }
    } catch (e) { Swal.fire({title:'خطأ', text:'فشل الحفظ', icon:'error', position:'center'}); }
  };

  const handleBulkInventoryUpdate = async (product, isAdding) => {
    const qty = Number(invBulkInputs[product.id]); if (!qty || qty <= 0) return Swal.fire({title:'تنبيه', text:'أدخل كمية صحيحة', icon:'warning', position:'center'});
    let newStock = Number(product.stock) + (isAdding ? qty : -qty); if (newStock < 0) return Swal.fire({title:'خطأ', text:'المخزون لا يكفي', icon:'error', position:'center'});
    try {
      ...
      if (res.ok) { Swal.fire({toast:true, position:'bottom-center', icon:'success', title:'تم تحديث الجرد', showConfirmButton:false, timer:1500}); setInvBulkInputs({ ...invBulkInputs, [product.id]: '' }); fetchAllData(); }
    } catch (e) {}
  };
  ...
  const handleCheckoutPOS = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        Swal.fire({ title: 'تم الاعتماد بنجاح!', icon: 'success', showCancelButton: true, confirmButtonText: 'طباعة الفاتورة 🧾', cancelButtonText: 'إغلاق', position:'center' }).then((result) => {
          if (result.isConfirmed) handlePrintReceipt(adminCart, total); setAdminCart([]); setEditingOrderId(null); setAdminView('orders'); fetchAllData();
        });
      }
  ...
  const handleImageUpload = (e, targetField) => {
    ...
    reader.onload = (event) => { const img = new Image(); img.src = event.target.result;
      img.onload = () => { const cvs = document.createElement('canvas'); cvs.width = 500; cvs.height = img.height * (500 / img.width); const ctx = cvs.getContext('2d'); ctx.drawImage(img, 0, 0, cvs.width, cvs.height); const compressedImage = cvs.toDataURL('image/jpeg', 0.6);
        if (targetField === 'worker') setWorkerForm({ ...workerForm, image: compressedImage }); else setFormData({ ...formData, image: compressedImage }); }; };
  };
  ...
  const handleRating = (type, name) => {
    Swal.fire({ title: `تقييم ${name}`, html: `<div style="font-size: 2.5rem; color: #f1c40f; cursor: pointer; display: flex; justify-content: center; gap: 10px;" id="star-container"><span class="star" data-val="1">☆</span><span class="star" data-val="2">☆</span><span class="star" data-val="3">☆</span><span class="star" data-val="4">☆</span><span class="star" data-val="5">☆</span></div><textarea id="rating-comment" placeholder="اكتب تجربتك هنا (اختياري)..." style="width: 90%; margin-top: 20px; padding: 15px; border-radius: 10px; border: 1px solid #ccc; font-family:inherit;"></textarea>`, didOpen: () => { const stars = document.querySelectorAll('.star'); let selectedValue = 0; stars.forEach(s => { s.addEventListener('click', (e) => { selectedValue = e.target.getAttribute('data-val'); stars.forEach(st => { st.innerHTML = st.getAttribute('data-val') <= selectedValue ? '⭐' : '☆'; }); }); }); }, showCancelButton: true, confirmButtonText: 'إرسال التقييم ✅', cancelButtonText: 'إلغاء' }).then((result) => { if (result.isConfirmed) { Swal.fire({title:'شكراً لك!', text:'تم استلام تقييمك بنجاح، رأيك يهمنا جداً.', icon:'success', position:'center'}); } });
  };
  ...
  // =========================================================================
  // 💻 5. واجهة العميل (Storefront) - الإصدار الفاخر V 3.0
  // =========================================================================
  ...
  return (
    <div className={`App client-theme ${showCart || selectedProduct || showWorkersHaraj ? 'no-scroll' : ''} ${darkMode ? 'dark-mode' : ''}`}>
      ...
      {/* شريط التقييمات المتحرك (Trust Marquee) */}
      <div className="marquee-container" style={{position:'fixed', bottom:0, width:'100%', zIndex:999}}>
        ...
        <div className="product-modal-overlay" onClick={() => setSelectedProduct(null)}>
          <div className="product-modal-content fade-in-up" onClick={e => e.stopPropagation()} style={{backgroundColor: darkMode ? '#1e1e1e':'#fff', color: darkMode?'#fff':'#000'}}>
            ...
            <div className="modal-body-split">
              <div className="m-img-side" style={{backgroundColor: darkMode?'#fff':'#fdfdfd'}}>{selectedProduct.is_sale && <div className="m-discount">🔥 عرض <span className="fire-anim">🔥</span></div>}<img src={selectedProduct.image || 'https://via.placeholder.com/300'} alt="" className="blend-image" /></div>
              <div className="m-details-side">
                ...
                <div className="m-price-box" style={{marginTop:'10px'}}><span className="m-now">{selectedProduct.price} ر.س</span>{selectedProduct.old_price > 0 && <span className="m-old">{selectedProduct.old_price} ر.س</span>}</div>
                <div style={{display:'flex', gap:'10px', marginBottom:'20px', flexWrap:'wrap'}}>{parseProductDetails(selectedProduct.details).warranty && <span style={{background:'rgba(52, 152, 219, 0.1)', color:'#3498db', padding:'5px 10px', borderRadius:'8px', fontWeight:'bold'}}>🛡️ ضمان {parseProductDetails(selectedProduct.details).warranty}</span>}{parseProductDetails(selectedProduct.details).color && <span style={{background:'rgba(155, 89, 182, 0.1)', color:'#9b59b6', padding:'5px 10px', borderRadius:'8px', fontWeight:'bold'}}>🎨 اللون: {parseProductDetails(selectedProduct.details).color}</span>}{parseProductDetails(selectedProduct.details).manufacturer && <span style={{background:'rgba(46, 204, 113, 0.1)', color:'#27ae60', padding:'5px 10px', borderRadius:'8px', fontWeight:'bold'}}>🏭 {parseProductDetails(selectedProduct.details).manufacturer}</span>}</div>
                <div className="m-desc-box"><h3 style={{color: darkMode?'#ddd':'var(--navy)'}}>المواصفات:</h3><div className="m-desc" style={{backgroundColor: darkMode?'#333':'#f9f9f9', color: darkMode?'#fff':'#555'}}>{parseProductDetails(selectedProduct.details).text || 'لا يوجد تفاصيل.'}</div></div>
                <button style={{background:'transparent', color:'var(--navy)', border:'2px solid var(--navy)', padding:'10px', borderRadius:'10px', fontWeight:'bold', marginBottom:'15px', cursor:'pointer', display:'block', width:'100%'}} onClick={() => window.open(`https://wa.me/?text=مرحباً، وش رأيك في هذا المنتج: ${selectedProduct.name} بسعر ${selectedProduct.price} ريال من متجر ${settings.shop_name}؟`)}>مشاركة المنتج للاستشارة 📤</button>
                {!selectedProduct.out_of_stock ? (<><div className="qty-controls" style={{justifyContent:'center', padding:'15px', marginBottom:'15px'}}><button onClick={() => handleProductQuantityChange(selectedProduct.id, 1)}>+</button><span style={{fontSize:'1.5rem', fontWeight:'bold', margin:'0 15px'}}>{itemQtys[selectedProduct.id] || 1}</span><button onClick={() => handleProductQuantityChange(selectedProduct.id, -1)}>-</button></div><button className="m-add-btn" onClick={() => { addToCart(selectedProduct); setSelectedProduct(null); }}>إضافة للسلة 🛒</button></>) : (<button className="m-add-btn disabled" disabled>🚫 نفدت الكمية</button>)}
              </div>
            </div>
          </div>
        </div>
      ...
      {showWorkersHaraj && (
        <div className="cart-overlay open" style={{background:'rgba(0,0,0,0.7)', backdropFilter:'blur(8px)'}}>
          <div className="cart-inner-container-large fade-in-up" style={{maxWidth:'800px', backgroundColor: darkMode ? '#1e1e1e':'#fff'}}>
             ...
             <div className="workers-filters" style={{padding:'15px', background: darkMode?'#222':'#f8f9fa', borderBottom:'1px solid #eee', display:'flex', gap:'10px', flexWrap:'wrap'}}><select value={harajRegion} onChange={e => {setHarajRegion(e.target.value); setHarajCity('');}} style={{flex:1, padding:'10px', borderRadius:'8px', border:'2px solid var(--gold)'}}><option value="">🔍 كل مناطق المملكة</option>{Object.keys(SAUDI_REGIONS).map((r, i) => <option key={i} value={r}>{r}</option>)}</select><select value={harajCity} onChange={e => setHarajCity(e.target.value)} style={{flex:1, padding:'10px', borderRadius:'8px', border:'2px solid var(--gold)'}} disabled={!harajRegion}><option value="">🏙️ المحافظات</option>{harajRegion && SAUDI_REGIONS[harajRegion].map((c, i) => <option key={i} value={c}>{c}</option>)}</select></div>
             <div className="cart-products-scroll" style={{background: darkMode?'#121212':'#fdfdfd'}}>
                 {visibleWorkers.length === 0 ? (<div className="empty-state"><h3 style={{color:darkMode?'#fff':'#333'}}>لا يوجد عمال متاحين.</h3></div>) : (
                    <div className="workers-public-grid" style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(250px, 1fr))', gap:'20px'}}>
                        {visibleWorkers.map(worker => (
                            <div key={worker.id} className="worker-public-card" style={{background: darkMode?'#222':'white', borderRadius:'15px', border: darkMode?'1px solid #444':'1px solid #eee', overflow:'hidden', boxShadow:'0 3px 10px rgba(0,0,0,0.05)', textAlign:'center', paddingBottom:'15px'}}>
                                <div style={{height:'100px', background:'var(--navy)', position:'relative'}}><div style={{width:'80px', height:'80px', borderRadius:'50%', border:'4px solid var(--gold)', overflow:'hidden', margin:'0 auto', position:'relative', top:'50px', background:'#fff'}}><img src={worker.image || 'https://via.placeholder.com/80'} alt="" style={{width:'100%', height:'100%', objectFit:'cover'}}/></div></div>
                                <div style={{marginTop:'60px', padding:'0 15px', color: darkMode?'#fff':'#000'}}>
                                    <h3 style={{margin:'0 0 5px 0'}}>{worker.name}</h3><span style={{background: darkMode?'#444':'#eee', padding:'3px 10px', borderRadius:'15px', fontSize:'0.85rem'}}>{worker.profession}</span>
                                    <div className="rating-stars" onClick={() => handleRating('worker', worker.name)} style={{marginTop:'10px'}}>⭐⭐⭐⭐⭐ <span style={{fontSize:'0.8rem', color:'#888'}}>(قيّم العامل)</span></div>
                                    <div style={{margin:'10px 0', fontSize:'0.9rem', color: darkMode?'#aaa':'#777'}}>📍 {worker.region} - {worker.city}</div><p style={{fontSize:'0.9rem', minHeight:'40px'}}>{worker.details}</p>
                                    <button onClick={() => window.open(`https://wa.me/${worker.phone}?text=مرحباً، رأيت إعلانك وأريد الاستفسار`)} style={{background:'#25d366', color:'white', border:'none', padding:'10px', borderRadius:'10px', fontWeight:'bold', cursor:'pointer', width:'100%'}}>تواصل واتساب 💬</button>
                                </div>
                            </div>
                        ))}
                    </div>
      ...
      {/* 🛒 السلة ذات المرحلتين (التعديل الجديد) */}
      {showCart && (
        <div className="cart-overlay open">
          <div className="cart-inner-container-large fade-in-up" style={{backgroundColor: darkMode?'#1e1e1e':'#fff', color: darkMode?'#fff':'#000'}}>
            <div className="cart-header-fixed">
              <h2>{checkoutStep === 1 ? 'سلة المشتريات 🛒' : 'إتمام الطلب 💳'}</h2>
              <button className="close-btn-x" onClick={() => {setShowCart(false); setCheckoutStep(1);}}>✕</button>
            </div>
            
            {checkoutStep === 1 ? (
              <>
                {/* المرحلة الأولى: مراجعة السلة */}
                {cartTotalAmount > 0 && remainingForFreeShipping > 0 && (<div style={{background:'#fff3cd', color:'#856404', padding:'10px', textAlign:'center', fontWeight:'bold', fontSize:'0.9rem'}}>أضف منتجات بقيمة {remainingForFreeShipping} ر.س لتوصيل مجاني! 🚚</div>)}
                {cartTotalAmount > 0 && remainingForFreeShipping <= 0 && (<div style={{background:'#d4edda', color:'#155724', padding:'10px', textAlign:'center', fontWeight:'bold', fontSize:'0.9rem'}}>حصلت على توصيل مجاني 🚚✨</div>)}
                
                <div className="cart-products-scroll" style={{backgroundColor: darkMode?'#121212':'#fdfdfd'}}>
                  {cart.length === 0 && <div className="empty-cart-msg">سلتك فارغة حالياً.</div>}
                  {cart.map((item, index) => (
                    <div key={index} className="cart-product-row" style={{backgroundColor: darkMode?'#222':'#fff', borderColor: darkMode?'#444':'#eee'}}>
                      ...
                      <div className="cart-p-details">
                        ...
                        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                          <div className="qty-controls-mini"><button onClick={() => { const n = [...cart]; n[index].qty++; setCart(n); }}>+</button><span>{item.qty}</span><button onClick={() => { const n = [...cart]; n[index].qty--; if(n[index].qty<=0) n.splice(index,1); setCart(n); }}>-</button></div>
                ...
                {cart.length > 0 && (
                  <div className="cart-floating-action" style={{backgroundColor: darkMode?'#1a1a1a':'#fff', borderColor: darkMode?'#333':'#eee'}}>
                    <div className="total-gold-box" style={{color: darkMode?'#fff':'var(--navy)'}}>الإجمالي: <span>{cartTotalAmount} ر.س</span></div>
                    <button className="btn-wa-confirm-giant" style={{background:'var(--navy)'}} onClick={() => setCheckoutStep(2)}>متابعة لإتمام الطلب 💳</button>
                  </div>
                )}
              </>
            ) : (
              <>
                {/* المرحلة الثانية: بيانات العميل وتأكيد الطلب */}
                <div className="cart-products-scroll" style={{backgroundColor: darkMode?'#121212':'#fdfdfd', padding:'20px'}}>
                   <button style={{background:'transparent', color:'var(--navy)', border:'1px solid var(--navy)', padding:'8px 15px', borderRadius:'8px', fontWeight:'bold', cursor:'pointer', marginBottom:'20px'}} onClick={() => setCheckoutStep(1)}>🔙 العودة للسلة</button>
                   
                   <div className="customer-info-box" style={{backgroundColor: darkMode?'#2a2a2a':'#e8f4f8', borderColor: darkMode?'#555':'#3498db', marginTop:0}}>
                     <h3 style={{marginTop:0, color: darkMode?'var(--gold)':'var(--navy)'}}>📍 بيانات التوصيل والدفع:</h3>
                     <input className="c-input" type="text" placeholder="الاسم الثلاثي (إجباري)" value={customerName} onChange={e => setCustomerName(e.target.value)} style={{backgroundColor: darkMode?'#444':'#fff', color: darkMode?'#fff':'#000'}} />
                     <input className="c-input" type="tel" placeholder="رقم الجوال (إجباري)" value={customerPhone} onChange={e => setCustomerPhone(e.target.value)} style={{backgroundColor: darkMode?'#444':'#fff', color: darkMode?'#fff':'#000'}} />
                     <input className="c-input" type="text" placeholder="رابط موقع التوصيل Google Maps (اختياري)" value={customerLocation} onChange={e => setCustomerLocation(e.target.value)} style={{backgroundColor: darkMode?'#444':'#fff', color: darkMode?'#fff':'#000'}} />
                     
                     <h4 style={{color: darkMode?'#ddd':'#555', marginTop:'15px'}}>طريقة الدفع المفضلة:</h4>
                     <select className="c-input" value={paymentMethod} onChange={e => setPaymentMethod(e.target.value)} style={{backgroundColor: darkMode?'#444':'#fff', color: darkMode?'#fff':'#000'}}>
                ...
                <div className="cart-floating-action" style={{backgroundColor: darkMode?'#1a1a1a':'#fff', borderColor: darkMode?'#333':'#eee'}}>
                  <div className="total-gold-box" style={{color: darkMode?'#fff':'var(--navy)'}}>المطلوب دفعه: <span>{cartTotalAmount} ر.س</span></div>
                  <button className="btn-wa-confirm-giant" onClick={handleCustomerSubmitOrder}>تأكيد وإرسال الطلب ✅</button>
                </div>
              </>
            )}
          </div>
