...
// 🗺️ قاعدة بيانات مناطق ومحافظات المملكة
const SAUDI_REGIONS = {
...
function App() {
  ...
  const [workerForm, setWorkerForm] = useState({ name: '', phone: '', details: '', image: '', region: '', city: '', profession: '' });
  const [editingWorker, setEditingWorker] = useState(null);
  const [newAdminForm, setNewAdminForm] = useState({ username: '', pin: '', role: 'موظف' });
  const [editingAdmin, setEditingAdmin] = useState(null);

  // Client Storefront States
  ...
  const fetchAllData = async () => {
    try {
      const [pRes, cRes, wRes, sRes, aRes, oRes] = await Promise.all([
        fetch(`${API_URL}/api/products`), fetch(`${API_URL}/api/categories`), fetch(`${API_URL}/api/workers`), 
        fetch(`${API_URL}/api/settings`), fetch(`${API_URL}/api/admins`), fetch(`${API_URL}/api/orders`)
      ]);
      const pData = await pRes.json(); const cData = await cRes.json(); const wData = await wRes.json();
      const sData = await sRes.json(); const aData = await aRes.json(); const oData = await oRes.json();
  ...
  const parseProductDetails = (detailsString) => {
    try { return JSON.parse(detailsString); } catch (e) { return { text: detailsString, color: '', warranty: '', badge: '', manufacturer: '' }; }
  };
  ...
  const handleLogin = async () => {
    if (!loginUsername || !loginPin) return Swal.fire('تنبيه', 'أدخل البيانات', 'warning');
    try {
    ...
    } catch (error) { Swal.fire('خطأ', 'مشكلة في الاتصال', 'error'); }
  };

  const handleChangeMyPassword = async () => {
    if (!newPasswordInput) return Swal.fire('تنبيه', 'أدخل الرمز الجديد', 'warning');
    try {
      ...
      if (res.ok) { Swal.fire({toast:true, position:'top-end', icon:'success', title:'تم التغيير', showConfirmButton:false, timer:1500}); setNewPasswordInput(''); fetchAllData(); }
    } catch (error) { Swal.fire('خطأ', 'حدث خطأ', 'error'); }
  };

  const handleAddCategory = async (isSub = false) => {
    const name = isSub ? newSubName : newMainName;
    if (!name) return Swal.fire('تنبيه', 'أدخل اسم القسم', 'warning');
    try {
      ...
      if (res.ok) { isSub ? setNewSubName('') : setNewMainName(''); Swal.fire({toast:true, position:'top-end', icon:'success', title:'تم إضافة القسم', showConfirmButton:false, timer:1500}); fetchAllData(); }
    } catch(e) { Swal.fire('خطأ', 'خطأ في الإضافة', 'error'); }
  };

  const handleDeleteCategory = async (id) => {
    if(window.confirm("حذف هذا القسم؟")) { await fetch(`${API_URL}/api/categories/${id}`, { method: 'DELETE' }); fetchAllData(); }
  };
  ...
  const handleSaveProduct = async () => {
    ...
    try {
      ...
      if (res.ok) {
        Swal.fire({toast:true, position:'center', icon:'success', title:'تم حفظ المنتج بنجاح!', showConfirmButton:false, timer:2000});
        setEditingItem(null); setFormData({ name: '', price: '', old_price: '', stock: '', details: '', manufacturer: '', color: '', warranty: '', badge: '', image: '', is_sale: false, out_of_stock: false }); fetchAllData();
      } else { Swal.fire('خطأ', 'فشل الحفظ', 'error'); }
    } catch (e) { Swal.fire('خطأ', 'فشل الاتصال بالسيرفر', 'error'); }
  };

  const handleDeleteProduct = async (id) => {
    if (window.confirm("حذف المنتج؟")) { await fetch(`${API_URL}/api/products/${id}`, { method: 'DELETE' }); fetchAllData(); }
  };

  const handleSaveWorker = async () => {
    if (!workerForm.name || !workerForm.phone || !workerForm.region || !workerForm.city) return Swal.fire('تنبيه', 'الاسم، الجوال، المنطقة، والمدينة إجبارية', 'warning');
    ...
    try {
      ...
      if (res.ok) {
        Swal.fire({toast:true, position:'center', icon:'success', title:'تم حفظ العامل', showConfirmButton:false, timer:2000});
        setWorkerForm({ name: '', phone: '', details: '', image: '', region: '', city: '', profession: '' }); setEditingWorker(null); fetchAllData();
      } else { Swal.fire('خطأ', 'لم يتم الحفظ', 'error'); }
    } catch(e) { Swal.fire('خطأ', 'مشكلة في الاتصال', 'error'); }
  };

  const handleDeleteWorker = async (id) => {
    if (window.confirm("حذف العامل؟")) { await fetch(`${API_URL}/api/workers/${id}`, { method: 'DELETE' }); fetchAllData(); }
  };

  const handleSaveAdmin = async () => {
    if (!newAdminForm.username || !newAdminForm.pin) return Swal.fire('تنبيه', 'الاسم والرمز السري مطلوبان', 'warning');
    ...
    try {
      ...
      if (res.ok) {
        Swal.fire({toast:true, position:'center', icon:'success', title:'تم حفظ الموظف', showConfirmButton:false, timer:2000});
  ...
  const handleDeleteAdmin = async (id, role) => {
    if (role === 'مدير') return Swal.fire('مرفوض', 'لا يمكن حذف حساب المدير!', 'error');
    if (window.confirm("حذف هذا الموظف؟")) { await fetch(`${API_URL}/api/admins/${id}`, { method: 'DELETE' }); fetchAllData(); }
  };
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      if (res.ok) { Swal.fire({toast:true, position:'center', icon:'success', title:'تم تحديث الإعدادات', showConfirmButton:false, timer:2000}); fetchAllData(); }
  ...
  const handleBulkInventoryUpdate = async (product, isAdding) => {
    ...
    if (!qty || qty <= 0) return Swal.fire('تنبيه', 'أدخل كمية صحيحة', 'warning');
  ...
  const handlePrintReceipt = (cartToPrint, total) => {
    ...
    const htmlContent = `
      <html dir="rtl"><head><style>body { font-family: 'Arial', sans-serif; width: 80mm; margin: 0 auto; padding: 10px; color: #000; text-align: center; font-size: 14px; } table { width: 100%; text-align: right; border-collapse: collapse; margin: 15px 0; font-size: 12px; } th, td { border-bottom: 1px dashed #ccc; padding: 5px 0; } .header h2 { margin: 0; font-size: 20px; } .total { font-size: 18px; font-weight: bold; text-align: left; margin-top: 10px; border-top: 2px solid #000; padding-top: 10px; }</style></head>
      <body><div class="header"><h2>${settings.shop_name}</h2><p>رقم التواصل: ${settings.phone}</p><p>التاريخ: ${new Date().toLocaleString('ar-SA')}</p></div><table><tr><th>الصنف</th><th>الكمية</th><th>السعر</th></tr>${cartToPrint.map(item => `<tr><td>${item.name}</td><td>${item.qty}</td><td>${item.price * item.qty}</td></tr>`).join('')}</table><div class="total">الإجمالي: ${total} ر.س</div><p style="margin-top: 20px;">شكراً لزيارتكم! ⚡</p></body></html>
    `;
  ...
  const handleCheckoutPOS = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        Swal.fire({ title: 'تم الاعتماد بنجاح!', icon: 'success', showCancelButton: true, confirmButtonText: 'طـبـاعـة الفـاتـورة 🧾', cancelButtonText: 'إغلاق' }).then((result) => {
  ...
  // ==========================================
  // 4. دوال التقييم والشراء (للعميل)
  // ==========================================
  const addToCart = (product, isClient = true) => {
    const targetCart = isClient ? cart : adminCart; const setTargetCart = isClient ? setCart : setAdminCart;
    const qtyToAdd = isClient ? (itemQtys[product.id] || 1) : 1;
    const existingIndex = targetCart.findIndex(item => item.id === product.id);
    if (existingIndex >= 0) { const newCart = [...targetCart]; newCart[existingIndex].qty += qtyToAdd; setTargetCart(newCart); } 
  ...
  const handleCustomerSubmitOrder = async () => {
    ...
    if (!customerName || !customerPhone) return Swal.fire('تنبيه', 'الرجاء إدخال الاسم ورقم الجوال', 'warning');
    const totalAmount = cart.reduce((sum, item) => sum + (item.price * item.qty), 0);
    Swal.fire({ title: 'جاري إرسال الطلب...', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
    try {
      const res = await fetch(`${API_URL}/api/orders`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ customer_name: customerName, customer_phone: customerPhone, cart_data: cart, total: totalAmount }) });
      if (res.ok) {
        Swal.fire({ title: 'شكراً لك! 🎉', text: 'تم استلام طلبك بنجاح، فريقنا سيتواصل معك في أقرب وقت لتأكيد التوصيل.', icon: 'success', confirmButtonText: 'متابعة التسوق' });
  ...
  // دالة التقييم الشاملة
  const handleRating = (type, name) => {
    Swal.fire({
      title: `تقييم ${name}`,
      html: `
        <div style="font-size: 2rem; color: #f39c12; cursor: pointer; display: flex; justify-content: center; gap: 5px;" id="star-container">
        ...
        <textarea id="rating-comment" placeholder="اكتب تعليقك هنا (اختياري)..." style="width: 100%; margin-top: 15px; padding: 10px; border-radius: 8px; border: 1px solid #ccc;"></textarea>
      `,
      didOpen: () => {
        ...
        stars.forEach(s => {
          s.addEventListener('click', (e) => {
            selectedValue = e.target.getAttribute('data-val');
            stars.forEach(st => { st.innerHTML = st.getAttribute('data-val') <= selectedValue ? '⭐' : '☆'; });
          });
        });
      },
      showCancelButton: true, confirmButtonText: 'إرسال التقييم', cancelButtonText: 'إلغاء'
    }).then((result) => {
      if (result.isConfirmed) {
        Swal.fire('شكراً لك!', 'تم استلام تقييمك بنجاح، رأيك يهمنا لتحسين خدماتنا.', 'success');
      }
    });
  };
  ...
  if (isAdminPanel) {
    ...
    return (
      <div className="admin-root">
        <style>{`
          .pulsing-bell { animation: ring 2s infinite; display: inline-block; transform-origin: top center; color: var(--red); font-size: 1.2rem; }
          .low-stock-dot { display: inline-block; width: 12px; height: 12px; background-color: var(--red); border-radius: 50%; animation: pulse 1s infinite; margin-right: 10px; }
        ...
        <aside className="sidebar-30">
          ...
          <nav className="side-nav">
            <button className={adminView === 'orders' ? 'active' : ''} onClick={() => setAdminView('orders')}>
              📥 الطلبات الواردة {pendingOrders.length > 0 && <span className="notification-badge"><span className="pulsing-bell">🔔</span> {pendingOrders.length}</span>}
            </button>
        ...
        <main className="content-70">
          {isManager && adminView !== 'pos' && adminView !== 'orders' && (
            <div className="admin-top-dashboard">
              ...
              <div className="dash-card highlight-card"><h4>الأرباح</h4><h2>{totalProfits} <span>ر.س</span></h2></div>
            </div>
          )}
            ...
            <div className="fade-in">
              <div className="panel-card mb-20">
                ...
                <table className="pro-table">
                  <thead><tr><th>رقم الطلب</th><th>العميل</th><th>الإجمالي</th><th>تحديث واتساب للعميل</th><th>إجراء</th></tr></thead>
                  <tbody>
                    ...
                    {pendingOrders.map(order => (
                      <tr key={order.id}>
                        ...
                        <td>
                          <button style={{background:'#f39c12', color:'#fff', border:'none', padding:'5px', borderRadius:'5px', cursor:'pointer', marginBottom:'5px', width:'100%'}} onClick={() => window.open(`https://wa.me/${order.customer_phone}?text=مرحباً ${order.customer_name}، جاري تجهيز طلبك رقم #${order.id} 📦`)}>إبلاغ: جاري التجهيز</button>
                          <button style={{background:'#27ae60', color:'#fff', border:'none', padding:'5px', borderRadius:'5px', cursor:'pointer', width:'100%'}} onClick={() => window.open(`https://wa.me/${order.customer_phone}?text=مرحباً ${order.customer_name}، طلبك رقم #${order.id} في الطريق إليك 🚚`)}>إبلاغ: في الطريق</button>
                        </td>
                        <td>
                          <button className="add-btn" style={{marginRight:'5px', marginBottom:'5px'}} onClick={() => { setAdminCart(order.cart_data); setEditingOrderId(order.id); setAdminView('pos'); }}>مراجعة بالفاتورة</button>
          ...
          {adminView === 'pos' && (
            <div className="pos-container fade-in">
              <div className="pos-products-section">
                <input type="text" className="pos-search" placeholder="🔍 ابحث عن منتج بالاسم (يدعم الباركود)..." value={posSearch} onChange={e => setPosSearch(e.target.value)}/>
              ...
              <div className="pos-cart-section">
                <h3>{editingOrderId ? `تعديل طلب #${editingOrderId}` : `سلة البيع (الكاشير)`}</h3>
                <div className="pos-cart-items">
                  {adminCart.map((item, index) => (
                    <div key={index} className="pos-cart-row">
                      ...
                      <div className="pos-qty-controls"><button onClick={() => { const n = [...adminCart]; n[index].qty++; setAdminCart(n); }}>+</button><span>{item.qty}</span><button onClick={() => { const n = [...adminCart]; n[index].qty--; if(n[index].qty<=0) n.splice(index,1); setAdminCart(n); }}>-</button></div>
                    </div>
                  ))}
                ...
                <div className="pos-checkout-area">
                  ...
                  <button className="pos-checkout-btn" onClick={handleCheckoutPOS}>اعتماد وطباعة الفاتورة 🧾</button>
                  {editingOrderId && <button className="del-btn-sq" style={{width:'100%', marginTop:'10px'}} onClick={() => { setEditingOrderId(null); setAdminCart([]); setAdminView('orders'); }}>إلغاء والعودة للطلبات</button>}
                </div>
              </div>
            </div>
          )}
          ...
          {adminView === 'inventory' && (
            <div className="fade-in">
              {!invMainCat ? (
                ...
                <div className="panel-card"><button className="back-btn" onClick={() => setInvMainCat(null)}>🔙 رجوع للأقسام الرئيسية</button><h2>📦 جرد الأقسام الفرعية لـ ({invMainCat.name})</h2><div className="folders-grid">{categories.filter(c => c.parent === invMainCat.name).map(cat => (<div key={cat.id} className="folder-card sub" onClick={() => setInvSubCat(cat)}><h3>{cat.name}</h3></div>))}</div></div>
              ) : (
                    ...
                    <tbody>
                      {products.filter(p => p.category === invSubCat.name).map(product => (
                        <tr key={product.id}>
                          ...
                          <td className="act-td"><div className="bulk-action-wrapper"><input type="number" className="bulk-input" placeholder="الكمية..." value={invBulkInputs[product.id] || ''} onChange={(e) => setInvBulkInputs({...invBulkInputs, [product.id]: e.target.value})}/><div className="bulk-buttons"><button className="btn-plus-bulk" onClick={() => handleBulkInventoryUpdate(product, true)}>إضافة (+)</button><button className="btn-minus-bulk" onClick={() => handleBulkInventoryUpdate(product, false)}>خصم (-)</button></div></div></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {!activeMainCat ? (
                ...
                <div className="panel-card"><button className="back-btn" onClick={() => { setActiveSubCat(null); setEditingItem(null); setFormData({ name: '', price: '', old_price: '', stock: '', details: '', manufacturer: '', color: '', warranty: '', badge: '', image: '', is_sale: false, out_of_stock: false }); }}>🔙 رجوع للأقسام</button>
                  ...
                  <div className="product-entry-form">
                    <div className="img-upload-box">{formData.image ? <img src={formData.image} alt="prod"/> : <div className="img-ph">صورة المنتج</div>}<label className="upload-label">رفع صورة 📸 <input type="file" onChange={(e) => handleImageUpload(e, 'product')} style={{display:'none'}}/></label></div>
                    <div className="data-entry-box">
                      ...
                      <div className="f-row mt-10">
                        ...
                        <select className="f-input" value={formData.badge} onChange={e => setFormData({...formData, badge: e.target.value})}>
                           <option value="">بدون شارة تميز</option><option value="best_seller">🏆 الأكثر مبيعاً</option><option value="new_arrival">✨ منتج جديد</option><option value="high_quality">⭐ جودة عالية</option>
                        </select>
                      </div>
                  ...
                  <div className="mini-products-list mt-30">
                    ...
                    {products.filter(p => p.category === activeSubCat.name).map(product => {
                      ...
                      return (
                        <div key={product.id} className="m-prod-row" onClick={() => { setEditingItem(product); setFormData({ ...product, color: parsed.color||'', warranty: parsed.warranty||'', badge: parsed.badge||'', manufacturer: parsed.manufacturer||'', details: parsed.text||'' }); }}>
          ...
          {adminView === 'workers' && (
            <div className="panel-card fade-in"><h2>👷‍♂️ إدارة العمال (تطبيق الحراج)</h2>
              ...
              <div className="data-entry-box" style={{flex: '1'}}>
                ...
                <div className="f-row">
                  <select className="f-input" value={workerForm.region} onChange={e => setWorkerForm({...workerForm, region: e.target.value, city: ''})}><option value="">اختر المنطقة...</option>{Object.keys(SAUDI_REGIONS).map((r, i) => <option key={i} value={r}>{r}</option>)}</select>
                  <select className="f-input" value={workerForm.city} onChange={e => setWorkerForm({...workerForm, city: e.target.value})} disabled={!workerForm.region}><option value="">اختر المحافظة/المدينة...</option>{workerForm.region && SAUDI_REGIONS[workerForm.region].map((c, i) => <option key={i} value={c}>{c}</option>)}</select>
                </div>
  ...
  // =========================================================================
  // 💻 5. واجهة العميل (Storefront) - الإصدار الفاخر V 2.0
  // =========================================================================
  ...
  return (
    <div className={`App client-theme ${showCart || selectedProduct || showWorkersHaraj ? 'no-scroll' : ''} ${darkMode ? 'dark-mode' : ''}`}>
      ...
      {/* CSS للوضع الليلي، تفريغ الصور، وحركة النار */}
      <style>{`
        ...
        .dark-mode .p-img-box { background-color: #fff; } 
        
        .blend-image { mix-blend-mode: multiply; object-fit: contain; width: 100%; height: 100%; padding: 0 !important; transform: scale(1.1); }
        .dark-mode .blend-image { mix-blend-mode: normal; transform: scale(1); } 
      ...
      <header className="royal-header" style={{boxShadow: '0 4px 15px rgba(0,0,0,0.1)'}}>
         ...
         <div style={{display:'flex', gap:'10px', alignItems:'center'}}>
             <button className="quote-btn-top desktop-only" onClick={() => window.open(`https://wa.me/${settings.phone}?text=مرحباً، لدي مشروع كامل وأريد تسعيرة خاصة للطلبات التالية:`)}>تسعيرة مشروع 🏗️</button>
          ...
          {clientMain && categories.filter(c => c.parent === clientMain).length > 0 && (
            <div className="client-sub-bar" style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
              ...
              <select style={{padding:'5px 10px', borderRadius:'10px', border:'1px solid var(--gold)', background:'transparent', color:'var(--navy)', fontWeight:'bold'}} value={sortType} onChange={e => setSortType(e.target.value)}>
                <option value="default">الترتيب الافتراضي 🔃</option><option value="price_asc">الأقل سعراً ⬇️</option><option value="price_desc">الأعلى سعراً ⬆️</option><option value="offers">العروض الخاصة 🔥</option>
              </select>
            </div>
          )}
        </>
          ...
          <div className="p-grid-royal">
            {processedProducts.map(product => {
              ...
              <div key={product.id} className="royal-p-card" onClick={() => setSelectedProduct(product)}>
                ...
                {/* تم إزالة الخلفية البيضاء باستخدام blend-image وتصفير الحواف ليكون المنتج كبير وواضح */}
                ...
                <div className="p-info-box">
                  ...
                  <div style={{fontSize:'0.8rem', color:'#888', marginBottom:'5px', minHeight:'18px'}}>
                  ...
                  <div className="price-area" style={{marginTop:'10px'}}>
      ...
      {/* نافذة عرض تفاصيل المنتج للعميل */}
      {selectedProduct && (
          ...
          <div className="product-modal-content fade-in-up" onClick={e => e.stopPropagation()} style={{backgroundColor: darkMode ? '#1e1e1e':'#fff', color: darkMode?'#fff':'#000'}}>
            ...
            <div className="modal-body-split">
              ...
              <div className="m-details-side">
                ...
                <div className="m-price-box" style={{marginTop:'10px'}}>
                ...
                <div className="m-desc-box">
                  ...
                  <div className="m-desc" style={{backgroundColor: darkMode?'#333':'#f9f9f9', color: darkMode?'#fff':'#555'}}>{parseProductDetails(selectedProduct.details).text || 'لا توجد تفاصيل إضافية مسجلة.'}</div>
                </div>
      ...
      {showWorkersHaraj && (
        <div className="cart-overlay open" style={{background:'rgba(0,0,0,0.7)', backdropFilter:'blur(8px)'}}>
          <div className="cart-inner-container-large fade-in-up" style={{maxWidth:'800px', backgroundColor: darkMode ? '#1e1e1e':'#fff'}}>
             <div className="cart-header-fixed"><h2>👷‍♂️ خدمات العمال والصيانة</h2><button className="close-btn-x" onClick={() => setShowWorkersHaraj(false)}>✕</button></div>
             <div className="workers-filters" style={{padding:'15px', background: darkMode?'#222':'#f8f9fa', borderBottom:'1px solid #eee', display:'flex', gap:'10px', flexWrap:'wrap'}}>
                <select value={harajRegion} onChange={e => {setHarajRegion(e.target.value); setHarajCity('');}} style={{flex:1, padding:'10px', borderRadius:'8px', border:'2px solid var(--gold)'}}><option value="">🔍 كل مناطق المملكة</option>{Object.keys(SAUDI_REGIONS).map((r, i) => <option key={i} value={r}>{r}</option>)}</select>
                <select value={harajCity} onChange={e => setHarajCity(e.target.value)} style={{flex:1, padding:'10px', borderRadius:'8px', border:'2px solid var(--gold)'}} disabled={!harajRegion}><option value="">🏙️ كل المحافظات والمدن</option>{harajRegion && SAUDI_REGIONS[harajRegion].map((c, i) => <option key={i} value={c}>{c}</option>)}</select>
             </div>
                        ...
                        {visibleWorkers.map(worker => (
                            <div key={worker.id} className="worker-public-card" style={{background: darkMode?'#222':'white', borderRadius:'15px', border: darkMode?'1px solid #444':'1px solid #eee', overflow:'hidden', boxShadow:'0 3px 10px rgba(0,0,0,0.05)', textAlign:'center', paddingBottom:'15px'}}>
                                ...
                                <div style={{marginTop:'60px', padding:'0 15px', color: darkMode?'#fff':'#000'}}>
                                    ...
                                    <button onClick={() => window.open(`https://wa.me/${worker.phone}?text=مرحباً، رأيت إعلانك في متجر ${settings.shop_name} وأريد الاستفسار عن خدماتك`)} style={{background:'#25d366', color:'white', border:'none', padding:'10px', borderRadius:'10px', fontWeight:'bold', cursor:'pointer', width:'100%'}}>تواصل واتساب 💬</button>
                                </div>
                            </div>
                        ))}
                    </div>
