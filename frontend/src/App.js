...
function App() {
  ...
  // Live Notifications Monitor
  useEffect(() => {
    if (orders.length > prevOrderCount && prevOrderCount !== 0 && isAdminPanel) {
      Swal.fire({toast:true, position:'top-end', icon:'info', title:'🔔 طلب جديد وصل!', showConfirmButton:false, timer:4000});
    }
  ...
  // ✅ تم حل مشكلة العمال بضمان إرسال جميع القيم الافتراضية
  const handleSaveWorker = async () => {
    ...
    try {
      ...
      if (res.ok) { Swal.fire({title:'نجاح', text:'تم حفظ العامل بامتياز', icon:'success', position:'center'}); setWorkerForm({ name: '', phone: '', details: '', image: '', region: '', city: '', profession: '' }); setEditingWorker(null); fetchAllData(); }
      else { Swal.fire({title:'خطأ', text:'السيرفر يرفض البيانات', icon:'error', position:'center'}); }
    } catch(e) { Swal.fire({title:'خطأ', text:'فشل الاتصال بالسيرفر', icon:'error', position:'center'}); }
  };
  ...
  // =========================================================================
  // 💻 5. واجهة العميل (Storefront) - الإصدار الفاخر V 4.0
  // =========================================================================
  ...
  const loyaltyPoints = Math.floor(cartTotalAmount / 100) * 5; // 5 نقاط لكل 100 ريال

  return (
    <div className={`App client-theme ${showCart || selectedProduct || showWorkersHaraj ? 'no-scroll' : ''} ${darkMode ? 'dark-mode' : ''}`}>
      
      {/* سحر التنسيق الجديد للجوال وإزالة الفراغات */}
      <style>{`
        ...
        .fire-anim { display: inline-block; animation: flame 0.8s infinite alternate; font-size: 1.2rem; } @keyframes flame { 0% { transform: scale(1) rotate(-5deg); opacity: 0.8; text-shadow: 0 0 5px orange; } 100% { transform: scale(1.2) rotate(5deg); opacity: 1; text-shadow: 0 0 15px red; } }
        ...
        .rating-stars { color: #f1c40f; font-size: 1.1rem; margin-top: 5px; cursor: pointer; }
        .marquee-container { background: #000; color: var(--gold); padding: 8px; font-weight: bold; font-size: 0.9rem; overflow: hidden; white-space: nowrap; border-top: 2px solid var(--gold); }
        ...
        /* تحسينات الجوال (إزالة الفراغات وتحويل النوافذ لـ Bottom Sheets) */
        @media (max-width: 768px) {
          .cart-overlay { align-items: flex-end !important; padding: 0 !important; }
          .cart-inner-container-large, .product-modal-content { width: 100% !important; margin: 0 !important; border-bottom-left-radius: 0 !important; border-bottom-right-radius: 0 !important; max-height: 90vh !important; padding: 15px !important; display: flex; flex-direction: column; overflow: hidden; }
          .modal-body-split { flex-direction: column !important; overflow-y: auto !important; padding-bottom: 80px !important; gap: 0 !important; }
          .m-img-side { height: 180px !important; margin-bottom: 10px !important; }
          .m-details-side { padding: 0 !important; }
          .sticky-mobile-buy { position: fixed; bottom: 0; left: 0; right: 0; background: var(--navy); padding: 15px; z-index: 1000; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 -4px 10px rgba(0,0,0,0.2); }
        }
      `}</style>
      ...
      <header className="royal-header" style={{boxShadow: '0 4px 15px rgba(0,0,0,0.1)'}}>
         ...
         <div className="search-bar-wrapper"><input placeholder="🔍 ابحث عن منتج..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} style={{borderRadius:'20px', padding:'10px 15px'}} /></div>
         <div style={{display:'flex', gap:'10px', alignItems:'center'}}>
             <button className="quote-btn-top desktop-only" onClick={() => window.open(`https://wa.me/${settings.phone}?text=مرحباً، لدي مشروع كامل وأريد تسعيرة خاصة:`)}>تسعيرة مشروع 🏗️</button>
             <button className="open-cart-large desktop-only" onClick={() => handleRating('store', settings.shop_name || 'المتجر')} style={{borderRadius:'20px', border:'2px solid var(--gold)', color:'var(--gold)', background:'transparent'}}>⭐ قيمنا</button>
             ...
             <button className="open-cart-large desktop-only" onClick={() => {setShowCart(true); setCheckoutStep(1);}} style={{borderRadius:'20px'}}>🛒 السلة <span style={{background:'var(--gold)', color:'#000', padding:'2px 8px', borderRadius:'10px', marginLeft:'5px'}}>{cart.length}</span></button>
      ...
      {!searchQuery && (
        <>
          <div className="client-main-bar"><button className={!clientMain ? 'active' : ''} onClick={() => {setClientMain(''); setClientSub('');}}>الكل</button>{mainCategoriesList.map(cat => (<button key={cat.id} className={clientMain === cat.name ? 'active' : ''} onClick={() => { setClientMain(cat.name); const sub = categories.filter(x => x.parent === cat.name); if(sub.length > 0) setClientSub(sub[0].name); else setClientSub(''); }}>{cat.name}</button>))}</div>
          {clientMain && categories.filter(c => c.parent === clientMain).length > 0 && (<div className="client-sub-bar" style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}><div style={{display:'flex', gap:'10px', overflowX:'auto', flex:1}}>{categories.filter(c => c.parent === clientMain).map(subCat => (<button key={subCat.id} className={clientSub === subCat.name ? 'active' : ''} onClick={() => setClientSub(subCat.name)}>{subCat.name}</button>))}</div><select style={{padding:'5px 10px', borderRadius:'10px', border:'1px solid var(--gold)', background:'transparent', color:'var(--navy)', fontWeight:'bold'}} value={sortType} onChange={e => setSortType(e.target.value)}><option value="default">الترتيب الافتراضي 🔃</option><option value="price_asc">الأقل سعراً ⬇️</option><option value="price_desc">الأعلى سعراً ⬆️</option><option value="offers">العروض الخاصة 🔥</option></select></div>)}
        </>
      )}
      
      <div className="gallery-container" style={{paddingBottom: '50px'}}>
        {processedProducts.length === 0 ? (<div className="empty-state"><h3 style={{color: darkMode?'#fff':'#333'}}>لا توجد منتجات هنا.</h3></div>) : (
          <div className="p-grid-royal">
            {processedProducts.map(product => {
              ...
              <div key={product.id} className="royal-p-card" onClick={() => setSelectedProduct(product)}>
                {product.is_sale && <div className="discount-badge" style={{display:'flex', alignItems:'center', gap:'5px'}}>ينتهي خلال {formatTime(timeLeft)} <span className="fire-anim">🔥</span></div>}
                ...
                <div className="p-info-box">
                  ...
                  <div style={{fontSize:'0.8rem', color:'#888', marginBottom:'5px', minHeight:'18px'}}>{parsedInfo.color && <span style={{display:'inline-block', border:'1px solid #ddd', padding:'2px 6px', borderRadius:'5px', marginRight:'5px'}}>اللون: <b>{parsedInfo.color}</b></span>}{parsedInfo.warranty && <span style={{display:'inline-block', border:'1px solid #ddd', padding:'2px 6px', borderRadius:'5px'}}>ضمان <b>{parsedInfo.warranty}</b></span>}</div>
                  <div className="rating-stars" onClick={(e) => { e.stopPropagation(); handleRating('product', product.name); }}>⭐⭐⭐⭐⭐</div>
                  <div className="price-area" style={{marginTop:'10px'}}><span className="now-price">{product.price} ر.س</span>{product.old_price > 0 && <span className="old-price">{product.old_price} ر.س</span>}{product.is_sale && <span className="fire-anim" style={{float:'left'}}>🔥</span>}</div>
                  <div className="action-area" style={{display:'flex', justifyContent:'space-between', alignItems:'center', gap:'10px'}}>
                    {!product.out_of_stock && (<div className="qty-controls" onClick={e => e.stopPropagation()}><button onClick={() => handleProductQuantityChange(product.id, 1)}>+</button><span style={{fontWeight:'bold', fontSize:'1.1rem'}}>{itemQtys[product.id] || 1}</span><button onClick={() => handleProductQuantityChange(product.id, -1)}>-</button></div>)}
                    <button className={`add-btn-p ${product.out_of_stock ? 'disabled' : ''}`} disabled={product.out_of_stock} onClick={(e) => { e.stopPropagation(); if (!product.out_of_stock) addToCart(product); }}>{product.out_of_stock ? 'نفدت' : 'أضف 🛒'}</button>
                  </div>
                </div>
              </div>
            )})}
      ...
      <button className="floating-wa-btn" style={{bottom:'50px'}} onClick={() => window.open(`https://wa.me/${settings.phone}`)}>💬</button>

      {/* نافذة المنتج المدمجة للموبايل (Bottom Sheet) */}
      {selectedProduct && (
        <div className="product-modal-overlay" onClick={() => setSelectedProduct(null)}>
          <div className="product-modal-content fade-in-up" onClick={e => e.stopPropagation()} style={{backgroundColor: darkMode ? '#1e1e1e':'#fff', color: darkMode?'#fff':'#000'}}>
            <button className="modal-close-x" onClick={() => setSelectedProduct(null)} style={{zIndex:100}}>✕</button>
            <div className="modal-body-split">
              ...
              <div className="m-details-side">
                <h2 style={{color: darkMode?'var(--gold)':'var(--navy)', margin:'5px 0'}}>{selectedProduct.name}</h2>
                <div className="rating-stars" onClick={() => handleRating('product', selectedProduct.name)}>⭐⭐⭐⭐⭐ <span style={{fontSize:'0.9rem', color:'#888', textDecoration:'underline'}}>(أضف تقييمك)</span></div>
                <div className="m-price-box" style={{marginTop:'5px', marginBottom:'10px'}}><span className="m-now">{selectedProduct.price} ر.س</span>{selectedProduct.old_price > 0 && <span className="m-old">{selectedProduct.old_price} ر.س</span>}</div>
                <div style={{display:'flex', gap:'5px', marginBottom:'10px', flexWrap:'wrap'}}>{parseProductDetails(selectedProduct.details).warranty && <span style={{background:'rgba(52, 152, 219, 0.1)', color:'#3498db', padding:'3px 8px', borderRadius:'8px', fontWeight:'bold', fontSize:'0.85rem'}}>🛡️ ضمان {parseProductDetails(selectedProduct.details).warranty}</span>}{parseProductDetails(selectedProduct.details).color && <span style={{background:'rgba(155, 89, 182, 0.1)', color:'#9b59b6', padding:'3px 8px', borderRadius:'8px', fontWeight:'bold', fontSize:'0.85rem'}}>🎨 اللون: {parseProductDetails(selectedProduct.details).color}</span>}{parseProductDetails(selectedProduct.details).manufacturer && <span style={{background:'rgba(46, 204, 113, 0.1)', color:'#27ae60', padding:'3px 8px', borderRadius:'8px', fontWeight:'bold', fontSize:'0.85rem'}}>🏭 {parseProductDetails(selectedProduct.details).manufacturer}</span>}</div>
                <div className="m-desc-box" style={{marginBottom:'0'}}><h3 style={{color: darkMode?'#ddd':'var(--navy)', margin:'5px 0'}}>المواصفات:</h3><div className="m-desc" style={{backgroundColor: darkMode?'#333':'#f9f9f9', color: darkMode?'#fff':'#555', padding:'10px'}}>{parseProductDetails(selectedProduct.details).text || 'لا يوجد تفاصيل.'}</div></div>
                <button style={{background:'transparent', color:'var(--navy)', border:'2px solid var(--navy)', padding:'8px', borderRadius:'10px', fontWeight:'bold', marginTop:'10px', cursor:'pointer', display:'block', width:'100%'}} onClick={() => window.open(`https://wa.me/?text=مرحباً، وش رأيك في هذا المنتج: ${selectedProduct.name} بسعر ${selectedProduct.price} ريال من متجر ${settings.shop_name}؟`)}>مشاركة للاستشارة 📤</button>
              </div>
            </div>
            {/* شريط الشراء العائم للموبايل */}
            <div className="sticky-mobile-buy hide-desktop">
              <div className="qty-controls" style={{background:'rgba(255,255,255,0.1)', color:'#fff', padding:'5px', borderRadius:'10px'}}><button onClick={() => handleProductQuantityChange(selectedProduct.id, 1)} style={{color:'#fff'}}>+</button><span style={{fontSize:'1.2rem', fontWeight:'bold', margin:'0 10px', color:'#fff'}}>{itemQtys[selectedProduct.id] || 1}</span><button onClick={() => handleProductQuantityChange(selectedProduct.id, -1)} style={{color:'#fff'}}>-</button></div>
              {!selectedProduct.out_of_stock ? (<button style={{background:'var(--gold)', color:'#000', border:'none', padding:'10px 20px', borderRadius:'10px', fontWeight:'bold', fontSize:'1.1rem'}} onClick={() => { addToCart(selectedProduct); setSelectedProduct(null); }}>أضف 🛒</button>) : (<button style={{background:'#555', color:'#fff', border:'none', padding:'10px', borderRadius:'10px'}} disabled>نفدت</button>)}
            </div>
          </div>
        </div>
      )}
      ...
      {showWorkersHaraj && (
        <div className="cart-overlay open" style={{background:'rgba(0,0,0,0.7)', backdropFilter:'blur(8px)'}}>
          <div className="cart-inner-container-large fade-in-up" style={{maxWidth:'800px', backgroundColor: darkMode ? '#1e1e1e':'#fff'}}>
             <div className="cart-header-fixed" style={{padding:'10px'}}><h2>👷‍♂️ حراج العمال</h2><button className="close-btn-x" onClick={() => setShowWorkersHaraj(false)}>✕</button></div>
             <div className="workers-filters" style={{padding:'10px', background: darkMode?'#222':'#f8f9fa', borderBottom:'1px solid #eee', display:'flex', gap:'5px', flexWrap:'wrap'}}><select value={harajRegion} onChange={e => {setHarajRegion(e.target.value); setHarajCity('');}} style={{flex:1, padding:'8px', borderRadius:'8px', border:'2px solid var(--gold)'}}><option value="">🔍 كل المناطق</option>{Object.keys(SAUDI_REGIONS).map((r, i) => <option key={i} value={r}>{r}</option>)}</select><select value={harajCity} onChange={e => setHarajCity(e.target.value)} style={{flex:1, padding:'8px', borderRadius:'8px', border:'2px solid var(--gold)'}} disabled={!harajRegion}><option value="">🏙️ المحافظات</option>{harajRegion && SAUDI_REGIONS[harajRegion].map((c, i) => <option key={i} value={c}>{c}</option>)}</select></div>
             <div className="cart-products-scroll" style={{background: darkMode?'#121212':'#fdfdfd'}}>
                 {visibleWorkers.length === 0 ? (<div className="empty-state"><h3 style={{color:darkMode?'#fff':'#333'}}>لا يوجد عمال.</h3></div>) : (
                    <div className="workers-public-grid" style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(250px, 1fr))', gap:'15px', padding:'10px'}}>
                        {visibleWorkers.map(worker => (
                            <div key={worker.id} className="worker-public-card" style={{background: darkMode?'#222':'white', borderRadius:'15px', border: darkMode?'1px solid #444':'1px solid #eee', overflow:'hidden', boxShadow:'0 3px 10px rgba(0,0,0,0.05)', textAlign:'center', paddingBottom:'15px'}}>
                                <div style={{height:'80px', background:'var(--navy)', position:'relative'}}><div style={{width:'70px', height:'70px', borderRadius:'50%', border:'3px solid var(--gold)', overflow:'hidden', margin:'0 auto', position:'relative', top:'40px', background:'#fff'}}><img src={worker.image || 'https://via.placeholder.com/80'} alt="" style={{width:'100%', height:'100%', objectFit:'cover'}}/></div></div>
                                <div style={{marginTop:'45px', padding:'0 10px', color: darkMode?'#fff':'#000'}}>
                                    <h3 style={{margin:'0 0 5px 0'}}>{worker.name}</h3><span style={{background: darkMode?'#444':'#eee', padding:'3px 10px', borderRadius:'15px', fontSize:'0.8rem'}}>{worker.profession}</span>
                                    <div className="rating-stars" onClick={() => handleRating('worker', worker.name)} style={{marginTop:'5px', fontSize:'0.9rem'}}>⭐⭐⭐⭐⭐ <span style={{fontSize:'0.7rem', color:'#888'}}>(قيّم)</span></div>
                                    <div style={{margin:'5px 0', fontSize:'0.85rem', color: darkMode?'#aaa':'#777'}}>📍 {worker.region} - {worker.city}</div><p style={{fontSize:'0.85rem', minHeight:'30px', margin:'5px 0'}}>{worker.details}</p>
                                    <div style={{display:'flex', gap:'5px', marginTop:'10px'}}>
                                      <button onClick={() => window.open(`https://wa.me/${worker.phone}?text=مرحباً، رأيت إعلانك وأريد الاستفسار`)} style={{background:'#25d366', color:'white', border:'none', padding:'8px', borderRadius:'8px', fontWeight:'bold', cursor:'pointer', flex:1}}>واتساب 💬</button>
                                      <button style={{background:'transparent', color:'var(--navy)', border:'1px solid var(--navy)', padding:'8px', borderRadius:'8px', fontWeight:'bold', cursor:'pointer', flex:1}}>معرض أعمالي 📸</button>
                                    </div>
                                </div>
                            </div>
                        ))}
      ...
      {showCart && (
        <div className="cart-overlay open">
          <div className="cart-inner-container-large fade-in-up" style={{backgroundColor: darkMode?'#1e1e1e':'#fff', color: darkMode?'#fff':'#000'}}>
            <div className="cart-header-fixed" style={{padding:'10px'}}>
              <h2 style={{margin:0}}>{checkoutStep === 1 ? 'سلة المشتريات 🛒' : 'إتمام الطلب 💳'}</h2>
              <button className="close-btn-x" onClick={() => {setShowCart(false); setCheckoutStep(1);}}>✕</button>
            </div>
            
            {checkoutStep === 1 ? (
              <>
                {cartTotalAmount > 0 && remainingForFreeShipping > 0 && (<div style={{background:'#fff3cd', color:'#856404', padding:'8px', textAlign:'center', fontWeight:'bold', fontSize:'0.85rem'}}>أضف منتجات بقيمة {remainingForFreeShipping} ر.س لتوصيل مجاني! 🚚</div>)}
                {cartTotalAmount > 0 && remainingForFreeShipping <= 0 && (<div style={{background:'#d4edda', color:'#155724', padding:'8px', textAlign:'center', fontWeight:'bold', fontSize:'0.85rem'}}>حصلت على توصيل مجاني 🚚✨</div>)}
                {cartTotalAmount > 0 && (<div style={{background: darkMode?'#333':'#e8f4f8', color: darkMode?'var(--gold)':'var(--navy)', padding:'5px', textAlign:'center', fontWeight:'bold', fontSize:'0.8rem'}}>🎁 ستكسب {loyaltyPoints} نقطة ولاء من هذا الطلب!</div>)}

                <div className="cart-products-scroll" style={{backgroundColor: darkMode?'#121212':'#fdfdfd', padding:'10px'}}>
                  {cart.length === 0 && <div className="empty-cart-msg">سلتك فارغة.</div>}
                  {cart.map((item, index) => (
                    <div key={index} className="cart-product-row" style={{backgroundColor: darkMode?'#222':'#fff', borderColor: darkMode?'#444':'#eee', padding:'10px', marginBottom:'10px'}}>
                      <img src={item.image || 'https://via.placeholder.com/80'} alt="" className="cart-p-img blend-image" style={{backgroundColor:'#fff', width:'60px', height:'60px'}} />
                      <div className="cart-p-details" style={{paddingRight:'10px'}}>
                        <div className="cart-p-title" style={{color: darkMode?'#fff':'var(--navy)', fontSize:'0.9rem', marginBottom:'5px'}}>{item.name}</div>
                        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                          <div className="qty-controls-mini" style={{transform:'scale(0.9)'}}><button onClick={() => { const n = [...cart]; n[index].qty++; setCart(n); }}>+</button><span>{item.qty}</span><button onClick={() => { const n = [...cart]; n[index].qty--; if(n[index].qty<=0) n.splice(index,1); setCart(n); }}>-</button></div>
                          <span className="cart-item-total" style={{fontSize:'0.9rem'}}>{item.price * item.qty} ر.س</span>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                {cart.length > 0 && (
                  <div className="cart-floating-action" style={{backgroundColor: darkMode?'#1a1a1a':'#fff', borderColor: darkMode?'#333':'#eee', padding:'10px'}}>
                    <div className="total-gold-box" style={{color: darkMode?'#fff':'var(--navy)', fontSize:'1.1rem', marginBottom:'10px'}}>الإجمالي: <span>{cartTotalAmount} ر.س</span></div>
                    <button className="btn-wa-confirm-giant" style={{background:'var(--navy)', padding:'12px', fontSize:'1rem'}} onClick={() => setCheckoutStep(2)}>متابعة لإتمام الطلب 💳</button>
                  </div>
                )}
              </>
            ) : (
              <>
                <div className="cart-products-scroll" style={{backgroundColor: darkMode?'#121212':'#fdfdfd', padding:'15px'}}>
                   <button style={{background:'transparent', color:'var(--navy)', border:'1px solid var(--navy)', padding:'5px 10px', borderRadius:'8px', fontWeight:'bold', cursor:'pointer', marginBottom:'15px'}} onClick={() => setCheckoutStep(1)}>🔙 رجوع</button>
                   <div className="customer-info-box" style={{backgroundColor: darkMode?'#2a2a2a':'#e8f4f8', borderColor: darkMode?'#555':'#3498db', marginTop:0, padding:'15px'}}>
                     <h3 style={{marginTop:0, color: darkMode?'var(--gold)':'var(--navy)', fontSize:'1.1rem'}}>📍 بيانات التوصيل والدفع:</h3>
                     <input className="c-input" type="text" placeholder="الاسم الثلاثي (إجباري)" value={customerName} onChange={e => setCustomerName(e.target.value)} style={{backgroundColor: darkMode?'#444':'#fff', color: darkMode?'#fff':'#000', padding:'10px', marginBottom:'10px'}} />
                     <input className="c-input" type="tel" placeholder="رقم الجوال (إجباري)" value={customerPhone} onChange={e => setCustomerPhone(e.target.value)} style={{backgroundColor: darkMode?'#444':'#fff', color: darkMode?'#fff':'#000', padding:'10px', marginBottom:'10px'}} />
                     <input className="c-input" type="text" placeholder="رابط موقع التوصيل Google Maps (اختياري)" value={customerLocation} onChange={e => setCustomerLocation(e.target.value)} style={{backgroundColor: darkMode?'#444':'#fff', color: darkMode?'#fff':'#000', padding:'10px', marginBottom:'10px'}} />
                     <h4 style={{color: darkMode?'#ddd':'#555', margin:'10px 0 5px 0'}}>طريقة الدفع المفضلة:</h4>
                     <select className="c-input" value={paymentMethod} onChange={e => setPaymentMethod(e.target.value)} style={{backgroundColor: darkMode?'#444':'#fff', color: darkMode?'#fff':'#000', padding:'10px'}}>
                ...
                <div className="cart-floating-action" style={{backgroundColor: darkMode?'#1a1a1a':'#fff', borderColor: darkMode?'#333':'#eee', padding:'10px'}}>
                  <div className="total-gold-box" style={{color: darkMode?'#fff':'var(--navy)', fontSize:'1.1rem', marginBottom:'10px'}}>المطلوب دفعه: <span>{cartTotalAmount} ر.س</span></div>
                  <button className="btn-wa-confirm-giant" style={{padding:'12px', fontSize:'1rem'}} onClick={handleCustomerSubmitOrder}>تأكيد وإرسال الطلب ✅</button>
                </div>
              </>
            )}
          </div>
