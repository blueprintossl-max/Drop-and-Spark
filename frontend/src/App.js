...
function App() {
  ...
  const [formData, setFormData] = useState({ name: '', price: '', old_price: '', stock: '', details: '', manufacturer: '', image: '', is_sale: false, out_of_stock: false, is_heavy: false, color: '', warranty: '', badge: '' });
  ...
  const parseProductDetails = (detailsString) => { try { return JSON.parse(detailsString); } catch (e) { return { text: detailsString, color: '', warranty: '', badge: '', manufacturer: '', is_heavy: false }; } };
  ...
  const handleSaveProduct = async () => {
    ...
    const advancedDetails = JSON.stringify({ text: formData.details || '', color: formData.color || '', warranty: formData.warranty || '', badge: formData.badge || '', manufacturer: formData.manufacturer || '', is_heavy: formData.is_heavy || false });
    ...
    try { const res = await fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); if (res.ok) { Swal.fire({title:'نجاح', text:'تم حفظ المنتج', icon:'success', position:'center'}); setEditingItem(null); setFormData({ name: '', price: '', old_price: '', stock: '', details: '', manufacturer: '', color: '', warranty: '', badge: '', image: '', is_sale: false, out_of_stock: false, is_heavy: false }); fetchAllData(); } } catch (e) { Swal.fire({title:'خطأ', text:'فشل الاتصال', icon:'error', position:'center'}); }
  };
  ...
  const handleCustomerSubmitOrder = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        Swal.fire({ title: '🎉 تم استلام طلبك بنجاح!', html: `رقم طلبك: <b style="color:var(--gold); font-size:1.5rem;">${orderId}</b><br><br>سنتواصل معك لتأكيد التوصيل.<br><br><span style="color:#27ae60; font-weight:bold;">💡 يمكنك تتبع حالة طلبك من قسم (تتبع طلباتي) باستخدام رقم جوالك.</span>`, icon: 'success', confirmButtonText: 'متابعة التسوق', position: 'center' });
    ...
    return (
      <div className="admin-root">
        ...
        <main className="content-70">
          ...
          {adminView === 'pos' && (<div className="pos-container fade-in"><div className="pos-products-section"><input type="text" className="pos-search" placeholder="🔍 ابحث للبيع..." value={posSearch} onChange={e => setPosSearch(e.target.value)}/><div className="pos-grid">{products.filter(p => !posSearch || p.name.includes(posSearch)).map(product => (<div key={product.id} className="pos-card" onClick={() => { if(product.stock > 0) addToCart(product, false); else Swal.fire({title:'نفدت الكمية',icon:'warning', position:'center'}); }}>{product.stock <= 0 && <div className="pos-out">نفدت</div>}<img src={product.image || 'https://via.placeholder.com/100'} alt=""/><h5 style={{height:'35px', overflow:'hidden'}}>{product.name}</h5><span className="pos-price" style={{color:'#e74c3c'}}>{product.price} ر.س</span><span className="pos-stock">المخزون: {product.stock}</span></div>))}</div></div><div className="pos-cart-section"><h3>{editingOrderId ? `تعديل طلب #${editingOrderId}` : `الكاشير`}</h3><div className="pos-cart-items">{adminCart.map((item, index) => (<div key={index} className="pos-cart-row"><div className="pos-cart-info"><b>{item.name}</b><span style={{color:'#e74c3c'}}>{item.price} ر.س</span></div><div className="pos-qty-controls"><button onClick={() => { const n = [...adminCart]; n[index].qty++; setAdminCart(n); }}>+</button><span>{item.qty}</span><button onClick={() => { const n = [...adminCart]; n[index].qty--; if(n[index].qty<=0) n.splice(index,1); setAdminCart(n); }}>-</button></div></div>))}{adminCart.length === 0 && <div className="pos-empty">السلة فارغة</div>}</div><div className="pos-checkout-area"><div className="pos-totals"><div className="p-row final"><span>المطلوب:</span><span style={{color:'#e74c3c'}}>{adminCart.reduce((sum, item) => sum + (item.price * item.qty), 0)} ر.س</span></div></div><button className="pos-checkout-btn" onClick={handleCheckoutPOS}>اعتماد وطباعة 🧾</button>{editingOrderId && <button className="del-btn-sq" style={{width:'100%', marginTop:'10px'}} onClick={() => { setEditingOrderId(null); setAdminCart([]); setAdminView('orders'); }}>العودة للطلبات</button>}</div></div></div>)}
          ...
          {adminView === 'categories' && (<div className="fade-in">{!activeMainCat ? (<div className="panel-card"><h2>1. الأقسام الرئيسية</h2><div className="add-row mb-20"><input placeholder="اسم القسم..." value={newMainName} onChange={e => setNewMainName(e.target.value)}/><button className="add-btn" onClick={() => handleAddCategory(false)}>إضافة 📁</button></div><div className="folders-grid">{mainCategoriesList.map(c => (<div key={c.id} className="folder-card main" onClick={() => setActiveMainCat(c)}><h3>{c.name}</h3><button className="del-btn-corner" onClick={(e) => { e.stopPropagation(); handleDeleteCategory(c.id); }}>حذف</button></div>))}</div></div>) : !activeSubCat ? (<div className="panel-card"><button className="back-btn" onClick={() => setActiveMainCat(null)}>🔙 رجوع</button><h2>2. الأقسام الفرعية لـ ({activeMainCat.name})</h2><div className="add-row mb-20"><input placeholder="قسم فرعي جديد..." value={newSubName} onChange={e => setNewSubName(e.target.value)}/><button className="add-btn" onClick={() => handleAddCategory(true)}>إضافة 📂</button></div><div className="folders-grid">{categories.filter(c => c.parent === activeMainCat.name).map(c => (<div key={c.id} className="folder-card sub" onClick={() => setActiveSubCat(c)}><h3>{c.name}</h3><button className="del-btn-corner" onClick={(e) => { e.stopPropagation(); handleDeleteCategory(c.id); }}>حذف</button></div>))}</div></div>) : (<div className="panel-card"><button className="back-btn" onClick={() => { setActiveSubCat(null); setEditingItem(null); setFormData({ name: '', price: '', old_price: '', stock: '', details: '', manufacturer: '', color: '', warranty: '', badge: '', image: '', is_sale: false, out_of_stock: false, is_heavy: false }); }}>🔙 رجوع</button><div className="path-header">{activeMainCat.name} ⬅️ {activeSubCat.name}</div><div className="product-entry-form"><div className="img-upload-box">{formData.image ? <img src={formData.image} alt="prod"/> : <div className="img-ph">صورة المنتج</div>}<label className="upload-label">رفع صورة 📸 <input type="file" onChange={(e) => handleImageUpload(e, 'product')} style={{display:'none'}}/></label></div><div className="data-entry-box"><input className="f-input full" placeholder="اسم المنتج (مثال: نافورة أسمنتية)..." value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})}/><div className="f-row"><input className="f-input" type="number" placeholder="السعر" value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})}/><input className="f-input" type="number" placeholder="السعر القديم" value={formData.old_price} onChange={e => setFormData({...formData, old_price: e.target.value})}/><input className="f-input" type="number" placeholder="المخزون" value={formData.stock} onChange={e => setFormData({...formData, stock: e.target.value})}/></div><div className="f-row mt-10"><input className="f-input" placeholder="اللون..." value={formData.color} onChange={e => setFormData({...formData, color: e.target.value})}/><input className="f-input" placeholder="الضمان..." value={formData.warranty} onChange={e => setFormData({...formData, warranty: e.target.value})}/><select className="f-input" value={formData.badge} onChange={e => setFormData({...formData, badge: e.target.value})}><option value="">بدون شارة</option><option value="best_seller">🏆 الأكثر مبيعاً</option><option value="new_arrival">✨ جديد</option><option value="high_quality">⭐ جودة عالية</option></select></div><input className="f-input full mt-10" placeholder="الشركة المصنعة..." value={formData.manufacturer} onChange={e => setFormData({...formData, manufacturer: e.target.value})}/><textarea className="f-input full mt-10" rows="2" placeholder="مواصفات..." value={formData.details} onChange={e => setFormData({...formData, details: e.target.value})}></textarea><div className="f-toggles"><button className={`t-btn ${formData.is_sale ? 'active-green' : ''}`} onClick={() => setFormData({...formData, is_sale: !formData.is_sale})}>🔥 عرض خاص</button><button className={`t-btn ${formData.out_of_stock ? 'active-out' : ''}`} onClick={() => setFormData({...formData, out_of_stock: !formData.out_of_stock})}>🚫 نفدت الكمية</button><button className={`t-btn ${formData.is_heavy ? 'active-orange' : ''}`} style={formData.is_heavy ? {background:'#f39c12', color:'#fff', borderColor:'#f39c12'} : {}} onClick={() => setFormData({...formData, is_heavy: !formData.is_heavy})}>🧱 منتج ثقيل (شحن خاص)</button><button className="save-btn" onClick={handleSaveProduct}>{editingItem ? 'تحديث المنتج 🔄' : 'حفظ بالمستودع ✅'}</button></div></div></div><div className="mini-products-list mt-30"><h3 style={{color:'var(--navy)'}}>المنتجات المسجلة:</h3>{products.filter(p => p.category === activeSubCat.name).map(product => { const parsed = parseProductDetails(product.details); return (<div key={product.id} className="m-prod-row" onClick={() => { setEditingItem(product); setFormData({ ...product, color: parsed.color||'', warranty: parsed.warranty||'', badge: parsed.badge||'', manufacturer: parsed.manufacturer||'', is_heavy: parsed.is_heavy||false, details: parsed.text||'' }); }}><img src={product.image || 'https://via.placeholder.com/50'} alt=""/><div style={{flex:1}}><b>{product.name}</b> {parsed.is_heavy && <span style={{fontSize:'0.7rem', background:'#f39c12', color:'#fff', padding:'2px 5px', borderRadius:'5px', marginLeft:'5px'}}>ثقيل</span>} <br/><small>{parsed.color && `لون: ${parsed.color}`}</small></div><span style={{color:'#e74c3c', fontWeight:'bold'}}>{product.price} ر.س</span><button className="del-btn-sq" onClick={(e) => { e.stopPropagation(); handleDeleteProduct(product.id); }}>حذف ❌</button></div>) })}</div></div>)}</div>)}
  ...
  // =========================================================================
  // 💻 5. واجهة العميل (Storefront) - الإصدار الفاخر V 7.0 (شحن ثقيل وتتبع طلبات)
  // =========================================================================
  ...
  const hasHeavyItems = cart.some(item => parseProductDetails(item.details).is_heavy);
  ...
  return (
    <div className={`App client-theme ${showCart || selectedProduct || showWorkersHaraj || showTrackOrder ? 'no-scroll' : ''} ${darkMode ? 'dark-mode' : ''}`}>
      ...
      <style>{`
        ...
        .p-info-box h4 { font-size: 1.6rem !important; }
        .add-btn-p { font-size: 1.3rem !important; padding: 15px 30px !important; border-radius: 15px !important; }
        ...
        @media (max-width: 768px) {
          ...
          .m-now { font-size: 2.8rem !important; font-weight: bold !important; color: #e74c3c !important; }
            ...
            {processedProducts.map(product => {
              ...
              <div key={product.id} className="royal-p-card" onClick={() => setSelectedProduct(product)}>
                ...
                <div className="p-info-box">
                  ...
                  <div style={{fontSize:'1rem', color:'#888', marginBottom:'10px', minHeight:'20px'}}>
                     {parsedInfo.color && <span style={{display:'inline-block', border:'1px solid #ddd', padding:'4px 8px', borderRadius:'8px', marginRight:'8px'}}>اللون: <b style={{color: darkMode?'#fff':'#000'}}>{parsedInfo.color}</b></span>}
                     {parsedInfo.warranty && <span style={{display:'inline-block', border:'1px solid #ddd', padding:'4px 8px', borderRadius:'8px'}}>ضمان <b style={{color: darkMode?'#fff':'#000'}}>{parsedInfo.warranty}</b></span>}
                     {parsedInfo.is_heavy && <span style={{display:'inline-block', background:'#f39c12', color:'#fff', padding:'4px 8px', borderRadius:'8px', marginTop:'5px'}}>🚛 منتج ثقيل (نقل خاص)</span>}
                  </div>
      ...
      <div className="marquee-container" style={{position:'fixed', bottom:0, width:'100%', zIndex:998}}>
         <div className="marquee-content">⭐⭐⭐⭐⭐ مقاول الرياض: جودة ممتازة وسرعة في التوصيل &nbsp;&nbsp;&nbsp;&nbsp; ⭐⭐⭐⭐⭐ أبو فهد: أسعار منافسة للأفياش والمفاتيح &nbsp;&nbsp;&nbsp;&nbsp; ⭐⭐⭐⭐⭐ المهندس علي: تعامل راقي وأسعار خرافية.</div>
      </div>
            ...
            <div className="modal-body-split">
              ...
              <div className="m-details-side">
                ...
                <div style={{display:'flex', gap:'10px', marginBottom:'20px', flexWrap:'wrap'}}>
                   {parseProductDetails(selectedProduct.details).warranty && <span style={{background:'rgba(52, 152, 219, 0.1)', color:'#3498db', padding:'5px 12px', borderRadius:'10px', fontWeight:'bold', fontSize:'1.1rem'}}>🛡️ ضمان {parseProductDetails(selectedProduct.details).warranty}</span>}
                   {parseProductDetails(selectedProduct.details).color && <span style={{background:'rgba(155, 89, 182, 0.1)', color:'#9b59b6', padding:'5px 12px', borderRadius:'10px', fontWeight:'bold', fontSize:'1.1rem'}}>🎨 اللون: {parseProductDetails(selectedProduct.details).color}</span>}
                   {parseProductDetails(selectedProduct.details).is_heavy && <span style={{background:'#f39c12', color:'#fff', padding:'5px 12px', borderRadius:'10px', fontWeight:'bold', fontSize:'1.1rem'}}>🚛 منتج ثقيل (نقل خاص)</span>}
                </div>
          ...
          <div className="cart-inner-container-large fade-in-up" style={{maxWidth:'600px', backgroundColor: darkMode ? '#1e1e1e':'#fff'}}>
             ...
             <div className="cart-products-scroll" style={{padding:'20px'}}>
                <div style={{marginBottom:'20px'}}>
                  <p style={{marginBottom:'10px', fontSize:'1.1rem', color: darkMode?'#ccc':'#555', fontWeight:'bold'}}>أدخل رقم الجوال الذي استخدمته في الطلب:</p>
                  <div style={{display:'flex', gap:'10px'}}>
                    <input type="tel" className="c-input" placeholder="05XXXXXXXX" value={trackPhoneInput} onChange={e => setTrackPhoneInput(e.target.value)} style={{flex:1, marginBottom:0, fontSize:'1.2rem', padding:'15px', backgroundColor: darkMode?'#444':'#fff', color: darkMode?'#fff':'#000', borderRadius:'10px', border:'2px solid var(--gold)'}} />
                     ...
                     {trackedOrdersList.map(o => (
                       <div key={o.id} style={{border:'2px solid var(--gold)', borderRadius:'15px', padding:'20px', background: darkMode?'#222':'#fdfdfd'}}>
                          ...
                          <div style={{display:'flex', alignItems:'center', gap:'15px'}}>
                            <strong style={{fontSize:'1.1rem', color: darkMode?'#fff':'#000'}}>حالة الطلب:</strong> 
                            ...
                            <div key={worker.id} className="worker-public-card" style={{background: darkMode?'#222':'white', borderRadius:'20px', border: darkMode?'1px solid #444':'1px solid #eee', overflow:'hidden', boxShadow:'0 5px 15px rgba(0,0,0,0.08)', textAlign:'center', paddingBottom:'20px'}}>
                                ...
                                <div style={{marginTop:'55px', padding:'0 20px', color: darkMode?'#fff':'#000'}}>
                                    ...
                                    <div style={{display:'flex', gap:'10px', marginTop:'20px'}}>
                                      ...
                                      <button style={{background:'transparent', color:'var(--navy)', border:'1px solid var(--navy)', padding:'12px', borderRadius:'15px', fontWeight:'bold', cursor:'pointer', flex:1, fontSize:'1.1rem'}}>معرض أعمالي 📸</button>
                                    </div>
                                </div>
                            </div>
                        ))}
          ...
          <div className="cart-inner-container-large fade-in-up" style={{backgroundColor: darkMode?'#1e1e1e':'#fff', color: darkMode?'#fff':'#000'}}>
            ...
            {checkoutStep === 1 ? (
              <>
                ...
                {hasHeavyItems && (<div style={{background:'#ffeaa7', color:'#d35400', padding:'15px', textAlign:'center', fontWeight:'bold', fontSize:'1.1rem', margin:'10px'}}>⚠️ سلتك تحتوي على منتجات ثقيلة (تحف/صبات). سيتم تحديد رسوم النقل الخاص بعد تأكيد الطلب، أو يمكنك الاستلام من المستودع مجاناً.</div>)}
                ...
                <div className="cart-products-scroll" style={{backgroundColor: darkMode?'#121212':'#fdfdfd', padding:'20px'}}>
                  ...
                  {cart.map((item, index) => {
                    const parsedItem = parseProductDetails(item.details);
                    return (
                    <div key={index} className="cart-product-row" style={{backgroundColor: darkMode?'#222':'#fff', borderColor: darkMode?'#444':'#eee', padding:'15px', marginBottom:'15px', borderRadius:'15px'}}>
                      ...
                      <div className="cart-p-details" style={{paddingRight:'15px'}}>
                        <div className="cart-p-title" style={{color: darkMode?'#fff':'var(--navy)', fontSize:'1.2rem', marginBottom:'10px'}}>{item.name} {parsedItem.is_heavy && <span style={{fontSize:'0.8rem', background:'#f39c12', color:'#fff', padding:'3px 8px', borderRadius:'8px'}}>🚛 ثقيل</span>}</div>
                        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                          ...
                          <span className="cart-item-total" style={{fontSize:'1.3rem', fontWeight:'bold', color:'#e74c3c'}}>{item.price * item.qty} ر.س</span>
                        </div>
                      </div>
                    </div>
                  )})}
                ...
                {cart.length > 0 && (
                  <div className="cart-floating-action" style={{backgroundColor: darkMode?'#1a1a1a':'#fff', borderColor: darkMode?'#333':'#eee', padding:'20px'}}>
                    <div className="total-gold-box" style={{color: darkMode?'#fff':'var(--navy)', fontSize:'1.5rem', marginBottom:'15px'}}>الإجمالي: <span style={{color:'#e74c3c'}}>{cartTotalAmount} ر.س</span></div>
              ...
              <>
                <div className="cart-products-scroll" style={{backgroundColor: darkMode?'#121212':'#fdfdfd', padding:'25px'}}>
                   ...
                   {hasHeavyItems && (<div style={{background:'#ffeaa7', color:'#d35400', padding:'15px', textAlign:'center', fontWeight:'bold', fontSize:'1.1rem', marginBottom:'15px', borderRadius:'10px'}}>⚠️ تذكير: طلبك يتضمن شحنات ثقيلة تحتاج نقل خاص.</div>)}
                ...
                <div className="cart-floating-action" style={{backgroundColor: darkMode?'#1a1a1a':'#fff', borderColor: darkMode?'#333':'#eee', padding:'20px'}}>
                  <div className="total-gold-box" style={{color: darkMode?'#fff':'var(--navy)', fontSize:'1.5rem', marginBottom:'15px'}}>المطلوب دفعه: <span style={{color:'#e74c3c'}}>{cartTotalAmount} ر.س</span></div>
